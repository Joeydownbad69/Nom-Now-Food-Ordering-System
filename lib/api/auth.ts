...
export async function registerUser(name: string, email: string, password: string): Promise<{ user: User; token: string }> {
  try {
    ...
    const existingUser = await db.getRow('SELECT * FROM users WHERE email = ?', [email]);
    ...
    // Get created user
    const user = await db.getRow('SELECT id, name, email, phone FROM users WHERE id = ?', [userId]);
